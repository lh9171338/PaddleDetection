...
class CopyAndPalse:
    ...
    def __init__(
        self, class_names, db_sampler_file, num_samples=5, max_retry=5, fade_epoch=0, epochs=0,
    ):
    ...
    def __call__(self, sample):
        ...
        epoch = sample["img_meta"]["epoch"]
        num_samples = self.num_samples.copy()
        if self.fade_epoch > 0 and epoch > self.fade_epoch:
            ratio = 1 - (epoch - self.fade_epoch) / (self.epochs - self.fade_epoch)
            for key, value in num_samples.items():
                num_samples[key] = int(value * ratio)
        ...
        for gt_name, gt_samples in self.gt_samples.items():
            ...
            if num < num_samples[gt_name]:
                num_sample = num_samples[gt_name] - num
